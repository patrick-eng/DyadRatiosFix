...
aggregate<- function(varname,date,index,ncases,mindate,maxdate,nperiods,nvar,aggratio,unit,miny,minper) {   #
  ...
  for (record in 1:nrec) { # MASTER LOOP THROUGH INPUT DATA, 1 TO NREC
    ...
    per<- findper(unit,curdate=as.integer(date[record]),mind,miny,minper,aggratio) #here we translate date into agg category
    if ((varname[record] !=  lv) || (per !=lp)) { #found a new period or variable name
      ...
      lp <- findper(unit,curdate=as.integer(date[record]),mind,miny,minper,aggratio)
...
sm.out<- optim(c(.75),smooth,method="L-BFGS-B",lower=0.3,upper=1)  #call smoother
